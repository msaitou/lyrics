// @/app/memories/chord-list/page.tsx
import Image from "next/image";
import React from "react";
const Page = () => {
  console.log("chord page!");
  const baseList: string[] = ["C", "C♯", "D", "D♯", "E", "F", "F♯", "G", "G♯", "A", "A♯", "B"]; // 12種
  const codeVarList: string[] = ["", "m", "7", "m7", "M7", "sus4", "7sus4", "add9", "m7♭5", "dim7", "6", "aug", "mM7", "m6"]; // 14種
                ...
                <tbody key={c + i}>
                  <tr>
                    {codeVarList.map(v => {
                      return (
                        <td key={c + v}>
                          {<Image priority src={`/${convImageName(c, v)}`} className="max-w-none" alt="" height={100} width={100} />}
                        </td>
                      );
                    })}
                  </tr>
